<?php
/**
 * RTB Payment Request email template.
 *
...
$payment_url = $email_context['payment_url'] ?? '';
$totals = $email_context['totals'] ?? [];
...
if (isset($totals['total'])) {
    $total_amount = number_format((float) $totals['total'], 2, ',', '.') . ' €';
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; color:#1f2933; line-height:1.6;">
    <h1 style="font-size:22px; margin:0 0 16px; color:#0b3d2e;">
        <?php echo esc_html($translate('rtb_payment.heading', [(string) ($experience['title'] ?? '')])); ?>
    </h1>

    <p style="margin:0 0 16px;">
        <?php echo esc_html($translate('rtb_payment.intro')); ?>
    </p>
    ...
    <?php if ($total_amount) : ?>
    <div style="padding:16px; border:2px solid #0b3d2e; border-radius:8px; margin-bottom:24px; background:#f0fdf4;">
        <p style="margin:0; font-size:18px; font-weight:600; color:#0b3d2e;">
            <?php echo esc_html($translate('rtb_payment.total_to_pay')); ?>: <?php echo esc_html($total_amount); ?>
        </p>
    </div>
    <?php endif; ?>
    ...
    <div style="text-align:center; margin:24px 0;">
        <a href="<?php echo esc_url($payment_url); ?>" 
           style="display:inline-block; padding:14px 32px; background:#0b3d2e; color:#ffffff; text-decoration:none; border-radius:8px; font-size:16px; font-weight:600;">
            <?php echo esc_html($translate('rtb_payment.pay_now_button')); ?>
        </a>
    </div>
    <p style="margin:16px 0 0; font-size:14px; color:#556987; text-align:center;">
        <?php echo esc_html($translate('rtb_payment.link_fallback')); ?>:<br>
        <a href="<?php echo esc_url($payment_url); ?>" style="color:#0b3d2e; word-break:break-all;">
    ...
    <p style="margin:0; font-size:14px; color:#556987;">
        <?php echo esc_html($translate('rtb_payment.footer_note')); ?>
    </p>
</div>
