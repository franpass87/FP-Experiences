...
- **Admin — Gift voucher**: lista CPT `fp_exp_gift_voucher` con banner gradiente FP (breadcrumb, `h1` screen-reader, titolo `h2`, badge versione, pulsante «Aggiungi» nel banner), card DMS attorno a filtri e tabella; colonna **Stato** con badge colorati (`fp-exp-dms-badge-*`).
- **CSS admin modulare**: token DMS in `variables.css`, nuovi fogli `dms-components.css`, `page-header-dms.css`, `gift-voucher-list.css` inclusi in `main.css` (build `fp-experiences-admin.min.css`); `admin.css` li importa per il fallback non minificato.

## [1.6.27] - 2026-04-04

### Fixed

- **Tools**: `tools/sync-source-to-dist-build.ps1` — rimossa riga duplicata/corrotta a fine script (output `Write-Host`).
